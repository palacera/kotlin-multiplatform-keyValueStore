...
class CacheAdapter(
    ...
    val cache: KottageStorage by lazy {
        val kottage = Kottage(
            name = "cache",
            directoryPath = when (config.isTemporary) {
    ...
    suspend inline fun <reified T : Any> put(cacheKey: CacheKey, value: T, cachePolicy: CachePolicy) =
        cache.put<CacheData<T>>(
            cacheKey.key, cacheData(value, cachePolicy), cachePolicy.getExpireDuration(),
        )
